...
var vents = {
    ...
    calculate_invoice: function () {
        ...
        this.items.descuento = this.items.subtotal * descuento;
        this.items.total = this.items.subtotal + this.items.iva - this.items.descuento;

        $('input[name="subtotal"]').val(this.items.subtotal.toFixed(2));
        $('input[name="ivacalc"]').val(this.items.iva.toFixed(2));
        $('input[name="descuentocalc"]').val(this.items.descuento.toFixed(2));
...
$(function () {
    ...
    // event submit
    $('#frmAlbar').on('submit', function (e) {
        e.preventDefault();

        if (vents.items.products.length === 0) {
            message_error('Debe al menos tener un item en su detalle de factura');
            return false;
        }
        ...
        submit_with_ajax(window.location.pathname, 'Notificación',
            '¿Estas seguro de realizar la siguiente acción?', parameters, function (response) {
                alert_action('Notificación', '¿Desea imprimir el detalle de la factura?', function () {
                    window.open('/erp/albar/invoice/pdf/' + response.id + '/', '_blank');
                    location.href = '/erp/albar/list/';
                }, function () {
                    location.href = '/erp/albar/list/';
                });
            });
